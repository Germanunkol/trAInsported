...
if DEDICATED then
	...
	function love.errhand(msg)
		...
		local width = love.graphics.getWidth() - 70
		local copied = false

		local nextSteps = LNG.error_steps

		local function draw()
			love.graphics.clear()

			love.graphics.setFont( FONT_BUTTON )
			love.graphics.printf( LNG.error_header, 60, 70, width )

			love.graphics.setFont( FONT_STANDARD )
			love.graphics.printf( p, 70, 120, width )

			love.graphics.setFont( FONT_BUTTON )
			love.graphics.printf( nextSteps, 60, love.graphics.getHeight() - 300, width )
